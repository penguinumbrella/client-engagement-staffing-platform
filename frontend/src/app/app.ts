...
export class App {
  ...
  private readonly globalSearchService =
    inject(GlobalSearchService);

  private readonly headerSearchService =
    inject(HeaderSearchService);

  private static readonly DARK_MODE_STORAGE_KEY =
    'theme-dark-mode';
  ...
  protected readonly darkMode = signal(
    localStorage.getItem(App.DARK_MODE_STORAGE_KEY) === 'true'
  );
  ...
  protected readonly searchResults =
    signal<GroupedSearchResults>(EMPTY_RESULTS);
  ...
  private readonly scrollIdleTimers =
    new WeakMap<EventTarget, ReturnType<typeof setTimeout>>();
    ...
    toObservable(this.searchQuery)
      .pipe(
        ...
        switchMap((query) => {
          ...
          // Consultants have no client/consultant roster to browse.
          // Their search box filters their own engagements locally.
          if (
            !role ||
            role === 'CONSULTANT' ||
  ...
  private onScroll(event: Event): void {
    ...
    clearTimeout(
      this.scrollIdleTimers.get(target)
    );
  ...
  protected onSearchInput(query: string): void {
    ...
    if (this.currentUser()?.role === 'CONSULTANT') {
      ...
      // Consultant search is a local live filter.
      this.headerSearchService.query.set(
        query.trim()
      );
