package com.skillstorm.staffing.exception;
...
class GlobalExceptionHandlerTest {
    ...
    void handleResponseStatus_mapsStatusAndReasonFromException() {
        ResponseStatusException ex = new ResponseStatusException(HttpStatus.NOT_FOUND, "Consultant 1 not found");

        ResponseEntity<ErrorResponse> response = handler.handleResponseStatus(ex);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
        assertThat(response.getBody().message()).isEqualTo("Consultant 1 not found");
    }

    @Test
    void handleBadRequest_returns400WithExceptionMessage() {
        IllegalArgumentException ex = new IllegalArgumentException("Unknown assignment status: Bogus");

        ResponseEntity<ErrorResponse> response = handler.handleBadRequest(ex);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody().message()).isEqualTo("Unknown assignment status: Bogus");
    }
    ...
    void handleValidation_joinsAllFieldErrorsIntoMessage() {
        ...
        when(bindingResult.getFieldErrors()).thenReturn(List.of(
                new FieldError("request", "consultantId", "consultantId is required"),
                new FieldError("request", "engagementId", "engagementId is required")
        ));
        ...
        assertThat(response.getBody().message())
                .isEqualTo("consultantId: consultantId is required; engagementId: engagementId is required");
    }
